...
export function AdminTicketWorkspace({ ticketId }: { ticketId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    if (ticketResponse.ok) setTicket(ticketBody.data.ticket);
    else setError(ticketBody.error?.message ?? "Unable to load ticket controls.");
    if (agentsResponse.ok) setAgents(agentsBody.data);
  }, [ticketId]);
  useEffect(() => {
    const timer = window.setTimeout(() => void load(), 0);
    return () => window.clearTimeout(timer);
  }, [load]);
  async function update(change: Partial<Ticket>) {
    setSaving(true);
    setError("");
    ...
    if (response.ok) setTicket(body.data);
    else setError(body.error?.message ?? "Unable to update this case.");
    setSaving(false);
  }
  return (
    <div className="admin-ticket-workspace">
      ...
      <div>
        <TicketConversation ticketId={ticketId} admin />
      </div>
    </div>
  );
}
