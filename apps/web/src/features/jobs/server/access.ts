...
export async function requireJobParticipant(jobId: string, userId: string) {
  const [job] = await db
    ...
    .from(jobs)
    .where(
      and(eq(jobs.id, jobId), or(eq(jobs.clientUserId, userId), eq(jobs.workerUserId, userId))),
    )
    .limit(1);
...
export async function requireMilestoneParticipant(milestoneId: string, userId: string) {
  const [record] = await db
    .select({ milestone: milestones, job: jobs })
    .from(milestones)
  ...
  return {
    ...record,
    role: record.job.clientUserId === userId ? ("CLIENT" as const) : ("WORKER" as const),
  };
}
