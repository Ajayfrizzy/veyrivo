...
export function parseSmallestUnit(value: string) {
  if (!/^[1-9][0-9]*$/.test(value))
    throw new Error("Amount must be a positive integer in the asset's smallest unit.");
  return BigInt(value);
}
...
export function calculateFees(subtotal: bigint) {
  ...
  const calculatedReserve = ceilDiv(subtotal * NETWORK_RESERVE_BPS, BPS_DENOMINATOR);
  const networkReserve =
    calculatedReserve > MIN_NETWORK_RESERVE ? calculatedReserve : MIN_NETWORK_RESERVE;
  return {
    subtotal,
    clientFee,
    workerFee,
