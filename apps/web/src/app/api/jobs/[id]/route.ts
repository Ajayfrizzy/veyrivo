...
export const GET = withApi(async (_request: Request, context: RouteContext<"/api/jobs/[id]">) => {
  const { id } = await context.params;
  const { user } = await requireUser();
  const access = await requireJobParticipant(id, user.id);
  const items = await db
    .select()
    .from(milestones)
    .where(eq(milestones.jobId, id))
    .orderBy(asc(milestones.sequence));
  return Response.json({ data: serialize({ ...access, milestones: items }) });
});
