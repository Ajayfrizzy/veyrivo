  ...
  async (request: Request, context: RouteContext<"/api/disputes/[id]/evidence">) => {
    ...
    if (input.fileId) {
      const [file] = await db
        .select()
        .from(proofFiles)
        .where(eq(proofFiles.id, input.fileId))
        .limit(1);
      if (!file) throw new ApiError(400, "FILE_NOT_FOUND", "The evidence file was not found.");
    }
    const [created] = await db
      .insert(disputeEvidence)
      .values({ disputeId: id, submittedBy: user.id, note: input.note, fileId: input.fileId })
      .returning();
