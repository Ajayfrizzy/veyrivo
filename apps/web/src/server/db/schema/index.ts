...
export const accountStatusEnum = pgEnum("account_status", ["PENDING_EMAIL_VERIFICATION", "ACTIVE", "RESTRICTED", "SECURITY_REVIEW", "SUSPENDED", "CLOSED"]);
export const identityStatusEnum = pgEnum("identity_status", ["NOT_STARTED", "PENDING", "VERIFIED", "REJECTED", "REVIEW_REQUIRED", "EXPIRED"]);
export const walletPurposeEnum = pgEnum("wallet_purpose", ["FUNDING", "PAYOUT", "BOTH"]);
export const walletStatusEnum = pgEnum("wallet_status", ["PENDING", "VERIFIED", "REPLACED", "LOCKED", "REVOKED"]);
export const agreementStatusEnum = pgEnum("agreement_status", ["DRAFT", "INVITED", "AWAITING_FUNDING", "FUNDED_AWAITING_ACCEPTANCE", "ACCEPTED", "IN_PROGRESS", "COMPLETED", "DECLINED", "EXPIRED", "CANCELLATION_PENDING", "CANCELLED", "DISPUTED", "SECURITY_HOLD", "REFUND_PENDING", "REFUNDED", "FAILED"]);
export const milestoneStatusEnum = pgEnum("milestone_status", ["PENDING", "ACTIVE", "PROOF_SUBMITTED", "UNDER_REVIEW", "REVISION_REQUESTED", "APPROVED", "RELEASE_PENDING", "RELEASED", "DISPUTED", "SECURITY_HOLD", "REFUND_PENDING", "REFUNDED", "CANCELLED", "FAILED"]);
export const operationStatusEnum = pgEnum("operation_status", ["PENDING", "PROCESSING", "CONFIRMED", "FAILED", "SECURITY_HOLD", "CANCELLED"]);
export const disputeStatusEnum = pgEnum("dispute_status", ["OPEN", "EVIDENCE_COLLECTION", "UNDER_REVIEW", "RESOLVED", "APPEALED", "CLOSED"]);
export const supportTicketStatusEnum = pgEnum("support_ticket_status", ["OPEN", "IN_PROGRESS", "WAITING_FOR_USER", "RESOLVED", "CLOSED"]);
export const supportTicketPriorityEnum = pgEnum("support_ticket_priority", ["LOW", "NORMAL", "HIGH", "URGENT"]);
export const supportTicketCategoryEnum = pgEnum("support_ticket_category", ["JOB", "PAYMENT", "WALLET", "SECURITY", "DISPUTE", "GENERAL"]);
export const supportSenderTypeEnum = pgEnum("support_sender_type", ["USER", "SUPPORT", "SYSTEM"]);
export const systemRoleEnum = pgEnum("system_role", ["USER", "SUPPORT", "DISPUTE_ADMIN", "RISK_ADMIN", "SUPER_ADMIN"]);
export const jobCategoryEnum = pgEnum("job_category", ["DESIGN", "DEVELOPMENT", "WRITING", "MARKETING", "DATA", "ADMIN", "OTHER"]);
export const jobListingStatusEnum = pgEnum("job_listing_status", ["DRAFT", "OPEN", "CLOSED", "AWARDED", "CANCELLED"]);
export const proposalStatusEnum = pgEnum("proposal_status", ["SUBMITTED", "WITHDRAWN", "REJECTED", "ACCEPTED"]);
...
export const authIdentities = pgTable("auth_identities", {
  ...
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
}, (table) => [uniqueIndex("auth_identity_provider_subject_unique").on(table.provider, table.providerSubject), index("auth_identity_user_idx").on(table.userId)]);
...
export const wallets = pgTable("wallets", {
  ...
  ...timestamps,
}, (table) => [uniqueIndex("wallet_user_network_address_unique").on(table.userId, table.network, table.address), index("wallet_user_idx").on(table.userId)]);
...
export const jobs = pgTable("jobs", {
  ...
  ...timestamps,
}, (table) => [uniqueIndex("jobs_reference_unique").on(table.reference), index("jobs_client_idx").on(table.clientUserId), index("jobs_worker_idx").on(table.workerUserId), check("jobs_amounts_nonnegative", sql`${table.subtotal} >= 0 AND ${table.clientFee} >= 0 AND ${table.networkReserve} >= 0`), check("jobs_worker_fee_valid", sql`${table.workerFeeBps} BETWEEN 0 AND 10000`)]);
...
export const milestones = pgTable("milestones", {
  ...
  ...timestamps,
}, (table) => [uniqueIndex("milestone_job_sequence_unique").on(table.jobId, table.sequence), check("milestone_amount_positive", sql`${table.amount} > 0`), check("milestone_review_days_valid", sql`${table.reviewPeriodDays} BETWEEN 1 AND 30`)]);

export const jobPayoutDestinations = pgTable("job_payout_destinations", {
  id: uuid("id").defaultRandom().primaryKey(),
...
export const jobListings = pgTable("job_listings", {
  ...
  check("job_listing_budget_valid", sql`${table.budgetMin} > 0 AND ${table.budgetMax} >= ${table.budgetMin}`),
]);

export const proposals = pgTable("proposals", {
  id: uuid("id").defaultRandom().primaryKey(),
  listingId: uuid("listing_id").notNull().references(() => jobListings.id, { onDelete: "cascade" }),
  workerUserId: uuid("worker_user_id").notNull().references(() => users.id),
...
export const proofFiles = pgTable("proof_files", {
  id: uuid("id").defaultRandom().primaryKey(),
  proofSubmissionId: uuid("proof_submission_id").notNull().references(() => proofSubmissions.id, { onDelete: "cascade" }),
  ...
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
}, (table) => [uniqueIndex("proof_file_storage_key_unique").on(table.storageKey), check("proof_file_size_valid", sql`${table.sizeBytes} > 0 AND ${table.sizeBytes} <= 26214400`)]);

export const reviews = pgTable("reviews", {
  id: uuid("id").defaultRandom().primaryKey(),
  milestoneId: uuid("milestone_id").notNull().references(() => milestones.id),
  proofSubmissionId: uuid("proof_submission_id").notNull().references(() => proofSubmissions.id),
...
export const disputes = pgTable("disputes", {
  ...
  resolvedAt: timestamp("resolved_at", { withTimezone: true }),
  ...timestamps,
}, (table) => [uniqueIndex("disputes_reference_unique").on(table.reference), index("disputes_job_idx").on(table.jobId)]);
...
export const disputeDecisions = pgTable("dispute_decisions", {
  id: uuid("id").defaultRandom().primaryKey(),
  ...
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
}, (table) => [check("dispute_split_complete", sql`${table.workerShareBps} >= 0 AND ${table.clientRefundBps} >= 0 AND ${table.workerShareBps} + ${table.clientRefundBps} = 10000`)]);
...
export const supportTickets = pgTable("support_tickets", {
  ...
  closedAt: timestamp("closed_at", { withTimezone: true }),
  ...timestamps,
}, (table) => [uniqueIndex("support_ticket_reference_unique").on(table.reference), index("support_ticket_user_idx").on(table.userId), index("support_ticket_queue_idx").on(table.status, table.priority, table.lastMessageAt), index("support_ticket_assignee_idx").on(table.assignedTo)]);
...
export const supportAttachments = pgTable("support_attachments", {
  ...
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
}, (table) => [uniqueIndex("support_attachment_storage_key_unique").on(table.storageKey), check("support_attachment_size_valid", sql`${table.sizeBytes} > 0 AND ${table.sizeBytes} <= 26214400`)]);
