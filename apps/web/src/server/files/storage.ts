...
export async function storePrivateFile(file: File) {
  if (!allowedTypes.has(file.type))
    throw new ApiError(
      ...
      "Only JPEG, PNG, WebP, PDF, and text files are allowed.",
    );
  if (file.size < 1 || file.size > MAX_FILE_BYTES)
    throw new ApiError(413, "FILE_SIZE_INVALID", "A proof file must be between 1 byte and 25 MB.");
  ...
  if (!target.startsWith(`${root}/`))
    throw new ApiError(400, "FILE_NAME_INVALID", "The file name is invalid.");
